---
name: obsidian-note
description: Use when user mentions "Obsidian", "옵시디언", "write as note", "save to notes", "노트로 저장", or /obsidian-note command. Do NOT use for TIL (use til), YouTube notes (use youtube-summarizer), or GenOS 작업 지식 캡처 (use genos-knowledge-capture).
---
...
팀 모드 절차:
1. **각도 분해** — 노트 주제를 N개 수집 각도로 나눈다 (예: 공식정의 / 실전사례 / 한계·반론, 또는 하위주제 A/B/C).
2. **서브에이전트 spawn** — `Agent` 도구로 N명 병렬 호출. 각 팀원 프롬프트에:
   - `subagent_type: general-purpose`, **read-only 수집만 — vault에 쓰지 말 것**
   - 담당 각도, 출력 포맷 `{핵심주장, 근거, 인용 URL}`, "`tavily_search` 우선 + 쿼리에 현재 연월 포함" 지시
3. **전원 완료 대기 (비협상)** — 한 명이라도 먼저 끝났다고 집필 시작 금지.
4. **교차확인 (가벼운 검증)** — lead가 N개 리포트를 대조:
   - 2명+ 합의 → 본문에 자신 있게 서술
   - 모순 → 노트에 "⚠️ 이견 있음"으로 양쪽 병기 (임의 채택 금지)
   - 단일 출처 → 약하게 서술하거나 "더 알아보기"로 이관
5. 수집·검증 결과를 들고 2단계로 진행.

> **비용 주의:** 팀 모드는 토큰 ~N배. 단순/익숙한 주제엔 기본(단일)을 쓴다.
> **충돌 회피:** 서브에이전트는 read-only 수집만, vault 쓰기(노트 1장)는 lead만 단독.
...
현재 팀 모드의 검증은 "lead 교차확인"(가벼움)이다. "가짜 정보 절대 안 됨" 수준이 필요해지면 1단계 팀 모드의 *수집 + 교차확인 코어*만 교체한다 (집필 이후는 불변):
- **교체 대상 (택1)**: 기성품 `deep-research` 스킬 호출 / Workflow JS (`parallel()` 수집 → 각 주장 N명 회의적 검증 투표 → loop-until-dry)
- **트리거 안**: `--verify strict` 또는 `--deep`
- **상세 설계**: `docs/superpowers/specs/2026-06-07-obsidian-note-team-mode-design.md`
