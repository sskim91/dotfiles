...
Commands:
  ...
  check-links-all         Check all vault notes for broken links (TSV: note<TAB>link)
...
find_notes() {
    find "$VAULT" -name "*.md" \
        ...
        -not -name "Vault-Semantic-Report*" \
        -not -name "Vault-Index.md" \
        -not -name "Vault-Log.md" \
        2>/dev/null
}
...
case "$1" in
    ...
    check-links-all) cmd_check_links_all ;;
