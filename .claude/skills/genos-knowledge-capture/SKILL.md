...
description: >-
  Use when GenOS 작업에서 얻은 재사용 가능한 지식(플랫폼 패턴·함정·고객사 결정)을
  개인 Obsidian vault(Projects/GenonAI)에 기록할 가치가 생겼거나, GenOS 작업 맥락에서
  사용자가 "지식 캡처", "vault에 정리/기록", "MOC", "읽는 순서",
  "field note", "/genos-knowledge-capture"를 요청할 때 사용.
  Do NOT use for: GenOS와 무관한 노트, 웹 리서치 기반 일반 개념 노트,
  단순 진행상황·세션 상태.
---
...
**① 온보딩 가속의 실체는 MOC + 읽는 순서다.** "그래프뷰가 예쁜 것"이 아니라 *작은 진입점(MOC)을 먼저 읽고, 거기서 오늘 필요한 노트를 발견한 뒤, 카테고리별 읽는 순서로 무엇부터 볼지 정하는 것*이다. AI는 Obsidian 그래프를 순회하지 못한다. 그래서 이 스킬은 늘 세 가지를 함께 한다: 원자적 노트 작성 + **MOC에 링크 추가** + **[[GenOS 카테고리별 읽는 순서]] 반영 검토**. MOC에 안 걸린 노트는 발견되지 않고, 읽는 순서에 안 걸린 노트는 처음 보는 사람이 언제 읽어야 할지 모른다.
...
핵심은 **용어를 다 설명하는 것보다, 독자가 궁금한 변화 한 가지를 끝까지 따라가게 하는 것**이다.
- 독자의 질문으로 시작한다. 예: "사이트 values에 replicas를 바꾸면 실제로 어디가 달라질까?"
- 입문·구조 설명은 [references/explanation-example.md](references/explanation-example.md)를 읽고, 한 예시의 **입력 → 읽는 주체 → 변환 → 결과 → 확인**을 이어 쓴다. YAML 전후 모습을 보여주고 누가 작성하는지, 값이 없으면 어떻게 되는지도 설명한다.
- 선행 개념은 그 예시를 이해할 만큼 먼저 풀어준다. 매 문단에 GenOS를 억지로 붙이지 않는다. 절이 끝날 때 실제 구조로 연결되면 된다. 의존성·내부 구현은 기본 흐름 뒤에 둔다.
- 기술 자체의 기능, 저장소의 관리 규칙, 현재 설정, 실제 관찰 결과를 구분한다. 문서와 코드가 충돌하면 어느 한쪽을 숨기지 말고 확인 범위를 적는다.
...
`<vault>/Projects/GenonAI/GenOS/GenOS 카테고리별 읽는 순서.md`를 열어 새 노트나 갱신된 노트가 들어갈 카테고리를 판단한다. 이 노트는 MOC를 대체하는 허브가 아니라 **MOC companion**이다. 역할을 섞지 않는다.

이 단계의 비협상은 **파일을 열고, 위치를 판단하고, 결과를 보고하는 것**이다. 실제 수정은 새 노트가 카테고리별 온보딩 순서에 들어가야 하거나, 기존 노트 갱신으로 읽는 순서의 의미가 달라질 때만 한다.

판단 순서:
1. 새/갱신 노트가 어떤 카테고리의 첫 독서 흐름에 들어가는지 정한다.
2. 해당 카테고리의 기존 순서에서 "이 노트를 읽기 전에 알아야 하는 노트"와 "이 노트 다음에 읽을 노트"를 잡는다.
3. 적절한 numbered list 위치에 `[[새 노트]]`를 추가하거나, 기존 노트 갱신만으로 순서 변경이 불필요하면 결과 보고에 불필요 사유를 남긴다.
...
### 7. 검증 + 결과 보고
- **검증**: Mermaid를 썼으면 렌더 검증, `related_notes`의 모든 `[[wikilink]]`는 vault 실존(또는 이번에 함께 만드는 노트)을 확인한다. 추가로 MOC 링크와 `GenOS 카테고리별 읽는 순서` 링크/불필요 사유를 확인한다.
- **설명 검토**: 독자가 "무엇을 바꾸면, 누가 읽고, 무엇이 달라지며, 어디서 확인하는가"를 답할 수 있는지 본문만으로 확인한다. 코드 조각만 있고 설명이 없으면 흐름 설명을 보충한다. 설정 예시와 실제 확인 결과를 혼동시키지 않는다.
...
| GenOS 외 일반 개념 요청 | 이 스킬을 쓰지 않고 `obsidian-note`나 다른 적절한 노트 스킬로 라우팅한다. |
| 입문자가 values에서 Pod까지 질문 | 용어 사전으로 시작하지 않고 한 값의 입력·변환·결과를 전후 예시로 보여준다. 렌더 성공과 실제 Pod 기동 확인은 구분한다. |
| 저장소 지침과 사이트 설정이 충돌 | 각 근거의 범위를 적고 미확인 부분을 남긴다. 관리 방침을 기술적 불가능으로 설명하지 않는다. |
...
- **카테고리별 읽는 순서를 빼먹는다.** MOC에 링크만 있으면 발견은 되지만, 처음 보는 사람은 언제 읽어야 하는지 모른다. *작성한 노트는 `GenOS 카테고리별 읽는 순서`에 넣을 위치를 항상 검토하고, 필요할 때만 수정하며, 넣지 않는다면 왜 순서 변경이 불필요한지 보고한다.*
...
- **입문자 친화를 용어 사전으로 만든다.** 정의가 모두 맞아도 값 하나가 어디로 가는지 연결되지 않으면 독자는 이해하지 못한다. 본 흐름에 필요한 개념만 먼저 설명하고, 한 예시를 끝까지 이어간다. 일반 개념 설명 자체는 필요하며 절 단위로 GenOS에 연결하면 된다.
- **쉽게 설명하려다 사실을 바꾼다.** "Helm은 kubectl apply를 해준다", "폐쇄망은 HTTP/OCI를 못 쓴다"처럼 비유나 로컬 선택을 제품 동작으로 단정하지 않는다. 비유 뒤에는 실제 처리 주체와 적용 범위를 적는다.
...
→ `GenOS/GenOS admin-api 외부 시스템 로그 연동 패턴.md` (tags: `type/pattern`, `backend/integration`), MOC "패턴·함정"에 링크, `GenOS 카테고리별 읽는 순서`의 Observability/operations 계열에 배치 검토.
