...
### 1단계: 토픽 슬러그 결정 및 기존 파일 확인

1. 세션의 핵심 주제를 영문 kebab-case 슬러그로 요약 (2~4단어)
   - 예: `api-refactor`, `auth-bugfix`, `nvim-lsp-setup`, `docker-migration`
2. 사용자에게 슬러그를 제안하고 확인받기
3. `_session/{topic-slug}.md` 파일이 이미 존재하는지 확인
   - **있으면** → 읽어서 이전 맥락 파악 (2단계에서 통합)
   - **없으면** → 새로 생성 예정

### 2단계: 상세 요약 생성

기존 핸드오프가 있으면 이전 맥락을 흡수하여 통합 작성합니다.
...
### 3단계: 파일 저장 및 Clipboard 복사

요약 완료 후:

1. **폴더 확인/생성**: 현재 작업 디렉토리에 `_session` 폴더가 있는지 확인
   - 없으면 `mkdir -p _session` 으로 생성
2. **파일 저장**: Write tool을 사용하여 `_session/` 폴더에 저장 (기존 파일이 있으면 덮어쓰기)
   - 파일명 형식: `_session/{topic-slug}.md`
   - 예시: `_session/api-refactor.md`, `_session/auth-bugfix.md`
3. **Clipboard 복사**: 저장된 파일의 정확한 경로로 clipboard에 복사
   ```bash
   cat _session/{topic-slug}.md | pbcopy
   ```
...
- 새 세션에서 사용 방법: `cat _session/{topic-slug}.md` 또는 `ls _session/`으로 작업 목록 확인
...
### Rolling 통합 시 압축 원칙
- **최근 2세션**: 상세하게 유지
- **그 이전**: 핵심 결정사항, 실패한 접근법, 아키텍처 결정만 남기고 압축
- **항상 유지**: 실패한 것들(What Didn't Work), 핵심 파일 목록, 남은 계획

### 완료된 토픽 정리
- 작업이 완전히 끝난 토픽은 사용자에게 삭제 여부 확인 후 `rm _session/{topic-slug}.md`
- `ls _session/`에 남아있는 파일 = 아직 진행 중인 작업
