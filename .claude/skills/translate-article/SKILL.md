...
Obsidian vault의 `Sources/Translations/{category}/`에 자동 저장하여, Wiki 노트에서 wikilink로 참조할 수 있게 합니다.
...
`/Users/sskim/Library/Mobile Documents/iCloud~md~obsidian/Documents/Note/Sources/Translations/{category}/`

번역본은 Obsidian vault의 `Sources/Translations/{category}/` 하위에 저장한다. vault 내부에 저장해야 다른 Wiki 노트에서 `[[파일명]]` 위키링크로 참조 가능하고, Obsidian 그래프뷰·백링크에 잡힌다.

`{category}`는 Step 2에서 판별한 카테고리 이름 (spring, python, ai 등).

카테고리 디렉토리가 없으면 새로 생성한다.
