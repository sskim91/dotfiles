...
| Approval, sandbox, permissions, bypass flags | `.codex/config.toml.example`, `zsh/functions.zsh:cdx` | Live defaults are intentionally permissive (`approval_policy = "never"`, `sandbox_mode = "danger-full-access"`); the `cdx` wrapper narrows this per-session with `--sandbox workspace-write --ask-for-approval on-request`, and `cdx -y` bypasses all guardrails. Treat bypass expansion as security review. |
...
| MCP server behavior | `~/.codex/config.toml:[mcp_servers.*]`, `.dotfiles/.codex/setup-mcp.sh`, `.mcp.json` | User has context7, playwright, tavily, brave-search, and youtrack (desktop-commander was removed in the 2026-07 MCP audit — duplicates native tools). Startup, OAuth, timeout, and tool schema changes are relevant. |
...
| Skills | `.codex/skills/*`, `~/.codex/skills/*`, plugin-provided skills | Skill discovery/frontmatter changes can affect reusable workflows. Keep as Tier 2 unless an existing skill clearly benefits. |
...
| Models, reasoning, context window, compaction | `.codex/config.toml.example`, `~/.codex/config.toml` | Live config uses `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"`; the tracked template keeps `high` as its baseline. `/review` inherits the session model unless explicitly overridden, and a manual compaction threshold remains. Re-check live config and session metadata before recommending model/default changes. |
