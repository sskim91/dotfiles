...
description: 웹 문서 URL을 받아 한국어 번역문을 작성할 때 사용한다. YouTube 영상은 youtube-summarizer를 사용한다.
---
...
`/Users/sskim/Library/Mobile Documents/iCloud~md~obsidian/Documents/Note/Sources/Translations/`

번역본은 Obsidian vault의 `Sources/Translations/` 아래에 **평면(flat)** 구조로 저장한다. 카테고리별 서브디렉토리를 만들지 않는다 — 파일명이 고유하면 Obsidian wikilink `[[파일명]]`이 위치 무관하게 해석되고, 분류는 frontmatter `tags`/`topics`로 충분하다.

Step 2에서 판별한 카테고리는 **경로가 아니라 frontmatter 태그**로 기록한다 (예: `tags: [til, spring]`).
...
- [ ] mermaid 사용 시: `npx -y @mermaid-js/mermaid-cli -i <파일>.mmd -o /tmp/mmd-check.svg`로 문법 검증 (exit 1이면 오류)
- [ ] ASCII 박스 대신 테이블/mermaid를 사용했는가?
- [ ] 출처가 명시되었는가?
- [ ] 파일명이 번역된 제목과 일치하는가?
- [ ] Post-write linking 실행했는가? (Obsidian vault 사용 시)
