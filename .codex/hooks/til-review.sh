...
extract_file_path() {
	local direct cwd patch candidate
	direct=$(printf '%s' "$INPUT" | jq -r '.tool_input.file_path // .tool_input.filePath // empty' 2>/dev/null)
	if [[ -n "$direct" ]]; then
		printf '%s\n' "$direct"
		return
	fi
	...
	while IFS= read -r candidate; do
		...
		if [[ "$candidate" =~ \.md$ ]] && [[ -f "$candidate" ]]; then
			printf '%s\n' "$candidate"
			return
		fi
	done < <(printf '%s\n' "$patch" | sed -nE 's/^\*\*\* (Add|Update) File: (.+)$/\2/p; s/^\*\*\* Move to: (.+)$/\1/p')
}
...
_antigravity_invoke() {
	...
	cd "$TIL_DIR" && {
		...
		agy --print "Review the TIL document and instructions provided on stdin. Follow the requested output format exactly." \
