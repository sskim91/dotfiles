...
# 결과를 00.Inbox/Vault-Lint-Report-{date}.md에 저장
#
# Max 구독 모델 — 추가 비용 없음
...
if [[ $PHASE1_EXIT -ne 0 || $PHASE2_EXIT -ne 0 ]]; then
  osascript -e "display notification \"Vault maintenance failed (lint:$PHASE1_EXIT, semantic:$PHASE2_EXIT). Check $LOG_FILE\" with title \"Vault Maintenance\""
fi
