...
- **Do not move the CLAUDE-omc.md sync back into a SessionStart hook** (relocated 2026-08-29). As a hook it ran every session to watch for drift that only a manual `claude plugin update` can cause (`autoUpdate: false`), and when a real bump finally landed (5.0.0 → 5.0.2) it synced exactly one line: the version marker. That cost two separate bug fixes. `ccpu` and `install.sh` now call `scripts/omc-companion-sync.sh` explicitly — the check is bound to its cause instead of polling.
